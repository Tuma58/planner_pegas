...
Рабочая серверная версия прототипа `Диспетчерская_ТК_21.html`: планер рейсов, вход по учетной записи, ролевой доступ, администрирование, SQLite и контур обмена с 1С через OData.
...
## Что перенесено из HTML ТК 21

Данные и нормативы:

- десять геозон, их координаты, города/регионы и приглушённая «промышленная» палитра ТК 21;
- фактический состав из 127 ТС, 1 651 рейса за июль 2026 и клиентская аналитика (109 заказчиков);
- расстояния и ставки по направлениям;
- полная экономика: НДС 22%/7% для ИП, дорожные, водитель, рефустановка, лизинг и накладные;
- статусы рейсов, причины отклонений, температурные режимы, кузова, шесть этапов заявки;
- роли логиста, ресурса, диспетчера, продаж, бухгалтерии и руководителя.

Интерфейс и функциональность:

- **тёмная тема** — переключатель в шапке, выбор хранится на сервере (`/api/preferences`);
- **drag&drop рейсов** по канве: перенос по дням и между сцепками, ручки длительности; рисование интервалов недоступности мышью;
- **маркер текущего дня**, легенда зон, чип «⚠ Требует решения N» с реестром исключений (конфликты, критичные, отклонённые);
- **карта геозон** — SVG-карта РФ с пузырями зон по реальным координатам и дугами потоков ТС по дням;
- **кабина руководителя** — круговые приборы КТГ/КВЛ/КИП (каскад по машино-дням считает сервер), «вид из лобового стекла» с планом выручки, LCD-панель P&L, светофоры, простой по причинам; план выручки и закрытие периодов;
- **6 печатных отчётов**: сводный, использование парка, экономика по типам ТС и по клиентам, отклонённые, история периодов (Печать/PDF);
- **доска продаж** — потребность от логистики (освобождающиеся сцепки с предложением обратного груза), бронирование с оценкой осуществимости, портфель со стадиями и назначением ТС;
- **календарь ресурса** — плашки-счётчики шести состояний парка, интервалы диспозиций на канве.

Экраны включаются вкладками по правам: Гант (всем), Продажи (`orders:write`), Ресурс (`fleet:write`), Руководитель (`reports:read`). Все параметры хранятся в БД и редактируются администратором, а не в JavaScript.

Исходные массивы ТК 21 зафиксированы в `src/tk20-data.json`. Повторно извлечь их из новой копии HTML можно командой:

```bash
node scripts/extract-tk20-data.mjs /path/to/Диспетчерская_ТК_21.html
```
