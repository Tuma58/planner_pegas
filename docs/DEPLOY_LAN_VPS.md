...
В LAN-режиме UFW разрешает SSH только из первоначального `LAN_CIDR`. Веб-порты принимает Nginx, а приложение до показа страницы входа проверяет адрес клиента по списку «Настройки → Сеть и доступ». Первоначально список заполняется значением `LAN_CIDR`; добавлять и удалять веб-подсети можно без повторного деплоя. Текущий IP администратора должен оставаться хотя бы в одной разрешённой подсети.
