...
# офисный VPN активен — на маке есть адрес из сети VPN 192.168.89.x.
# VPN поднялся — маршрут добавляется через шлюз VPN 192.168.89.170;
# VPN упал — маршрут удаляется, чтобы трафик к 100.100.10.x не утекал
# в чужой шлюз.
#
...
GATEWAY=192.168.89.170

vpn_ip=$(/sbin/ifconfig 2>/dev/null | /usr/bin/grep -o 'inet 192\.168\.89\.[0-9]*' | /usr/bin/head -1)
