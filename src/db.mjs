...
CREATE TABLE IF NOT EXISTS driver_attendance (
  ...
  marked_by TEXT REFERENCES users(id), marked_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(driver_id, day)
);
CREATE INDEX IF NOT EXISTS idx_driver_attendance_day ON driver_attendance(day);
...
CREATE TABLE IF NOT EXISTS chat_hidden (
  ...
  hidden_after INTEGER NOT NULL DEFAULT 0,
...
function migrateColumns(db) {
  ...
  // Telegram-уведомления: chat_id привязанного мессенджера и режим
  ...
  // Нормативы транзитного времени: 50 км/ч, 2 ч на грузовую операцию,
  ...
  if (calculationRow) {
    ...
    // Цель по воротам выгрузки, ч (этап 3, решение руководителя 22.09):
    // диагностика «дорогих ворот»; превышение у договорных клиентов —
    // претензия, у остальных — задание продажам.
    if (calculation.gateTargetHours == null) {
      calculation.gateTargetHours = 6;
      db.prepare(`UPDATE settings SET value_json=? WHERE key='calculation'`)
        .run(JSON.stringify(calculation));
    }
  ...
  // Норматив грузовой операции снижен с 3 до 2 часов (решение руководителя,
  // 07.08.2026): обновляем сохранённую настройку, если админ её не менял,
  // и перепланируем не начатые рейсы по новой формуле — с учётом
  // промежуточных пунктов заявки (операции = 2 + заезды) и окна клиента.
  if (!db.prepare(`SELECT 1 FROM app_meta WHERE key='transit_replan_v2'`).get()) {
    const calcRow = db.prepare(`SELECT value_json FROM settings WHERE key='calculation'`).get();
    const calc = calcRow ? JSON.parse(calcRow.value_json) : {};
    if (Number(calc.handlingHoursPerOperation) === 3) {
      calc.handlingHoursPerOperation = 2;
      db.prepare(`UPDATE settings SET value_json=? WHERE key='calculation'`)
        .run(JSON.stringify(calc));
    }
    const speed = Number(calc.techSpeedKmh || 50);
    const perOperation = Number(calc.handlingHoursPerOperation || 2);
    const factor = Number(calc.transitFactor || 1.5);
    const plans = db.prepare(`SELECT t.id,t.starts_at,t.distance_km,o.window_to,o.via_json
      FROM trips t LEFT JOIN orders o ON o.trip_id=t.id WHERE t.status='plan'`).all();
    const update = db.prepare(`UPDATE trips SET ends_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`);
    const clearStops = db.prepare(`DELETE FROM trip_stops WHERE trip_id=?
      AND actual_arrival IS NULL AND actual_departure IS NULL
    ...
    for (const trip of plans) {
      let viaCount = 0;
      try { viaCount = (JSON.parse(trip.via_json || '[]') || []).length; } catch { viaCount = 0; }
      const operations = 2 + viaCount;
      const transitMs = (Number(trip.distance_km || 0) / speed + operations * perOperation)
        * factor * 3_600_000;
      const endMs = Math.max(Date.parse(trip.starts_at) + transitMs,
        trip.window_to ? Date.parse(trip.window_to) : 0);
      if (!Number.isFinite(endMs)) continue;
  ...
  // Вахтовый график водителя: N дней работы / M отдыха от даты начала
  // рабочего периода; пусто — без вахты (считается рабочим всегда).
  ensure('drivers', 'shift_on', 'INTEGER');
  ensure('drivers', 'shift_off', 'INTEGER');
  ensure('drivers', 'shift_anchor', 'TEXT');
  // Должность сотрудника для отчётности (план/факт): не влияет на права —
  ...
  // Метка «зависание в пути уже подсвечено»: сторож runHangWatch шлёт
  // сигнал по рейсу один раз (кейс Винторга: 123 ч стоянки молча).
  ensure('trips', 'hang_alert_at', 'TEXT');
  // Рейс назначен зелёным коридором автоназначения: метка для контроля
  // качества — отмена/замена такого рейса человеком сигналит руководителю.
  ensure('trips', 'auto_assigned', 'INTEGER NOT NULL DEFAULT 0');
  // Честный пробег рейса: CAN-одометр Пилота за интервал «выезд → выгрузка»
  ...
  // Данные водителя и ТС направлены грузоотправителю: без них машину не
  // пускают на погрузку (пропуск на КПП). Параллельная отметка подготовки,
  // НЕ звено чек-листа — не всем клиентам данные нужны, и жёсткий шаг
  ...
  // Стойкая связь слота с карточкой клиента: имя остаётся для показа,
  // а id переживает переименование. Досветка старых строк — linkCustomerIds.
  ensure('delivery_slots', 'customer_id', 'TEXT REFERENCES customers(id)');
  // Вид плеча-спота: 'sell' — настоящий спот, задача продаж найти груз;
  // 'attach' — гарантированное плечо сетки, его закрывает логист заявкой.
  // Продажам показываются только 'sell': гарантированное первое плечо
  // в их заданиях только путало.
  ensure('route_spots', 'kind', "TEXT NOT NULL DEFAULT 'sell'");
  db.prepare(`UPDATE route_spots SET kind='attach'
    WHERE candidates LIKE 'плечо сетки%' AND kind='sell'`).run();
  ...
  ensure('demurrage_claims', 'reason', "TEXT NOT NULL DEFAULT ''");
  // Договорной простой клиента (этап 3 проекта «Скорости», 22.09):
  // ставка ₽/ч и бесплатные часы в профиле. ПУСТО = у клиента простой
  // в договоре не заведён — автоматика претензий его не трогает
  // (решение руководителя «не всех под одно»), глобальные настройки
  // остаются только аналитикой.
  ensure('customer_profiles', 'demurrage_rate', 'REAL');
  ensure('customer_profiles', 'demurrage_free_hours', 'REAL');
