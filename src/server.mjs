...
const SUBJECT_ZONES = [
  // ВАЖНО: \w в JS не матчит кириллицу — окончания субъектов ловим [а-яё].
  // До 09.2026 субъектный резолв из-за этого молчал («Омская обл» не матчилась).
  ['Дом', /(пензенск|мордови|саратовск|тамбовск)[а-яё]*\s*(обл|край|респ)/i],
  ['Москва', /(московск|тульск|калужск|рязанск)[а-яё]*\s*обл/i],
  ['Москва', /\sМО\s*$/],
  ['Питер', /(ленинградск|новгородск|вологодск|тверск)[а-яё]*\s*обл/i],
  ['Питер', /санкт-петербург/i],
  ['Золотое кольцо', /(нижегородск|владимирск|ивановск|костромск|ярославск)[а-яё]*\s*обл/i],
  ['Самара', /(самарск|ульяновск|оренбургск)[а-яё]*\s*обл/i],
  ['Самара', /(татарстан|марий эл|чуваш)/i],
  ['Урал', /(свердловск|челябинск|кировск)[а-яё]*\s*обл|пермск[а-яё]*\s*край|удмурт|башкорт|башкири/i],
  ['Новосибирск', /новосибирск[а-яё]*\s*обл|\sНвСиб\s*$/i],
  ['Омск', /омск[а-яё]*\s*обл/i],
  ['Кузбасс', /кемеровск[а-яё]*\s*обл|кузбасс/i],
  ['Томск', /томск[а-яё]*\s*обл/i],
  ['Тюмень', /тюменск[а-яё]*\s*обл|ханты|югра|ямал/i],
  ['Алтай', /алтайск[а-яё]*\s*край|респ[а-яё]{0,6}\s*алтай/i],
  ['Красноярск', /красноярск[а-яё]*\s*край|хакас/i],
  ['Восток', /иркутск[а-яё]*\s*обл|бурят|забайкал/i],
  ['Черноземье', /(воронежск|липецк|курск|белгородск|брянск|орловск)[а-яё]*\s*обл/i],
  ['Юг', /(ростовск|волгоградск|астраханск)[а-яё]*\s*обл|(краснодарск|ставропольск)[а-яё]*\s*край|карачаево/i],
  ['Запад', /(смоленск|псковск)[а-яё]*\s*обл/i]
];
...
// Матрица кузовов — справка руководителя 10.09: ВЕСЬ парк — рефрижераторы.
// Тушевоз = реф + подвесы полутуш, вместимость 33 паллетоместа;
// Паллет 33 = реф на 33 паллеты; Паллет 41 = реф на 41 паллету;
// Допельшток = реф на 33 паллеты + груз вторым ярусом. Температурные и
// безрежимные грузы возит ЛЮБОЙ тип; спецтребование заявки сужает выбор:
// подвесы — только Тушевоз, 41 паллета по полу — ТОЛЬКО Паллет 41
// (уточнение руководителя 10.09: Допельшток 41 по полу не везёт),
// второй ярус — только Допельшток.
const BODY_COMPAT = {
  'Рефрижератор': ['Тушевоз', 'Паллет 33', 'Паллет 41', 'Допельшток'],
  'Изотерм': ['Тушевоз', 'Паллет 33', 'Паллет 41', 'Допельшток'],
  'Тушевоз': ['Тушевоз'],
  'Паллет 33': ['Паллет 33', 'Паллет 41', 'Допельшток', 'Тушевоз'],
  'Паллет 41': ['Паллет 41'],
...
function applyDriverStep(chatId, stopId, phase) {
  ...
  return applyDriverStepFor(driver, stopId, phase);
}
function applyDriverStepFor(driver, stopId, phase) {
...
// Прогноз прибытия от водителя (общее ядро для каналов): запись
// driver_eta, подтверждение через reply(text), уведомление диспетчеру
// при прогнозе позже плана.
function applyDriverEtaFor(driver, stopId, hours, reply) {
  const stop = db.prepare(`SELECT s.*, t.vehicle_id FROM trip_stops s
    JOIN trips t ON t.id = s.trip_id WHERE s.id=?`).get(stopId);
  if (!stop || !Number.isFinite(hours)) return;
  const owns = stop.vehicle_id === driver.vehicle_id ||
    driverForTrip({ vehicle_id: stop.vehicle_id, id: stop.trip_id })?.id === driver.id;
  if (!owns) return;
  ...
  reply(null, `Принял: примерно к ${mskEta} (МСК). Диспетчер видит прогноз.`);
...
function splitEastZone() {
  try {
    if (db.prepare(`SELECT value FROM app_meta WHERE key='east_zone_split_v2'`).get()) return;
    ...
    db.prepare(`INSERT INTO app_meta(key,value) VALUES('east_zone_split_v2',?)`).run(String(rebound));
...
async function api(request, response, url) {
  ...
  // ── Отчёт эксплуатации парка ИЗ ДАННЫХ ПЛАНЕРА (15.09, независимо от 1С) ──
  // Канон КИП (принят руководителем 15.09, единый для планера и 1С-версии):
  //   на линии  = факт «на линию» → факт выгрузки (+ перегоны);
  //   под грузом = факт убытия с погрузки → факт ПРИБЫТИЯ на выгрузку;
  //   стоянка у клиента после прибытия — потеря КИП, не «под грузом».
  //   КТГ = 1 − ремонт/календарь; КВЛ = линия/(календарь − ремонт);
  //   КИП = под грузом/линия; КОЭФ = КТГ × КВЛ × КИП.
  if (request.method === 'GET' && pathname === '/api/park-report') {
    const user = requirePermission(request, response, 'reports:read');
    if (!user) return;
    const from = String(url.searchParams.get('from') || '').slice(0, 10);
    const to = String(url.searchParams.get('to') || '').slice(0, 10);
