...
// Расчёт от цели 153 млн/мес и штата ОДНОЙ смены (работа в две смены):
// 59 рейсов/сутки → ~30 на смену; продажи 2 чел/смена, логист 1,
// диспетчер 2. Норматив = на активную смену сотрудника.
export const STAFF_PLANS = {
  sales: { label: 'Продажи', metrics: [
    ['orderCreate', 'Заявок внесено', 17],
    ['ordersSum', 'Сумма внесённого, ₽', 1_500_000],
    ['orderAssign', 'Назначено ТС', 8]
  ] },
  logist: { label: 'Логист', metrics: [
    ['orderAssign', 'Назначено ТС', 30],
    ['dispatchSteps', 'Подтверждений/шагов', 30],
    ['orderUpdate', 'Правок заявок', 10]
  ] },
  dispatcher: { label: 'Диспетчер', metrics: [
    ['dispatchSteps', 'Шагов чек-листа', 60],
    ['stopFacts', 'Фактов на линии', 50],
